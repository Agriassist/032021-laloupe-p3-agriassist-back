...
const findManyByAgriculteurId = (id) => {
  const sql =
  'SELECT c.id, c.name, c.lastname, c.identifiant, c.phone, c.picture_profile, c.email FROM concessionnaire c JOIN partenariat p ON c.id = p.concessionaire_id JOIN agriculteur a ON a.id = p.agriculteur_id WHERE a.id = ?';
  return connection.promise().query(sql, [id]);
};
